...
class AlphaModel(nn.Module):#进行视频分割的UNet模型，输入视频，输出每个对象的MASK。
    ...
    def _pred_to_output(self, x): #这个函数的主要作用是考虑层间遮挡，但对单层数据来说用处不大
        ...
        ## v(M-1) = (1 - o(1)) * ... * (1 - o(M-2))
        ## v(M) = 0
        vis = torch.cumprod(1 - occ, dim=1) # 沿着ID=1的维度计算累积乘积
        '''
            vis有两层: 0号层全1、1号层全0
            每一层与上一层的结果再进行相乘
        ...
        acc = vis[:, :-1] * occ[:, 1:]  # (B, M-1, ...) #该层显现并且前面的几层都不可见(前景层真正的可见概率)
        ...
        # exit(0)
        weights = torch.cat([(1 - bg) * acc, bg], dim=1) #只有背景没有前景的时候这两个通道的内容也不一致 #acc是一层全1的数据
