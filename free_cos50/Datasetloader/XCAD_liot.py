...
class Config():
    def __init__(config):
        ...
        config.vessel3D=False
        config.logname="log"
        config.pseudo_label=False
        config.inputType="LIOT"
        config.dePhase=0
        pass
config = Config()
...
from free_cos50.Datasetloader.torch_LIOT import trans_liot, trans_NewLiot, trans_NewLiot2#, trans_liot_region, \
...
class DatasetXCAD_aug(data.Dataset):
    ...
    def __init__(self, datapath_img, datapath_gt):
    # def __init__(self, benchmark, datapath, split, img_mode, img_size, supervised):
        super(DatasetXCAD_aug, self).__init__()
        # print("benchmark, datapath, split, img_mode, img_size, supervised",benchmark, datapath, split, img_mode, img_size, supervised)
        benchmark = "XCAD_LIOT"
        datapath = "" #"./logs/FreeCOS46.log/xca_dataset_video"
        split = "test", 
        img_mode = "crop", 
        img_size = 256 
        ...
        if self.supervised == 'supervised':
            if self.split == 'train':  # 有监督的训练
                ...
                self.img_path = datapath_img #os.path.join(datapath, 'test', 'img')  # ./Data/XCAD/test/img #真实数据
                self.ann_path = datapath_gt  #os.path.join(datapath, 'test', 'gt')  # ./Data/XCAD/test/gt  #人工标签
