...
class MaskWarpLoss(nn.Module):#根据光流图预测的下一帧要与真实的下一帧一致
    ...
    def forward(self, batch_in, batch_out, split=False):
        ...
        """
        gap = self.gap # 1
        masks = batch_out["masks"].to("cuda")  # (B, M, 1, H, W) # [9, 2, 1, 128, 128] #分割的结果
        ...
        if gap > 0: #正向光流
            ...
            occ_map = batch_in["occ"][0].to("cuda") #occ.shape=[9, 1, 128, 128] #前向遮挡图
            # 不知道这个遮挡图是个啥
            ok = ok[:-gap]#去除最后一个false
            F12 = flow[:-gap].permute(0, 2, 3, 1)  # 0 ... B-1-gap #[8,128,128,2] #有效的光流图视频
            O12 = occ_map[:-gap].to("cuda") #[8,1,128,128] #遮挡图视频
            # M1 = masks[:-gap, :, 0, ...]  # 0 ... B-1-gap #[8, 2, 128, 128] #去除最后一张MASK图
            # M2 = masks[gap:, :, 0, ...]   # gap ... B-1   #[8, 2, 128, 128] #去除第一张MASK图
            M1 = masks[:-gap]  # 0 ... B-1-gap #[8, 2, 128, 128] #去除最后一张MASK图
            M2 = masks[gap:]   # gap ... B-1   #[8, 2, 128, 128] #去除第一张MASK图
        else: #逆向光流
            ok, flow = batch_in["bck"]
            occ_map = batch_in["disocc"][0].to("cuda")
            ok = ok[-gap:]
            F12 = flow[-gap:].permute(0, 2, 3, 1)  # gap ... B-1
            O12 = occ_map[-gap:].to("cuda")
            # M1 = masks[-gap:, :, 0, ...]  # gap ... B-1
            # M2 = masks[:gap, :, 0, ...]  # 0 ... B-1-gap
            M1 = masks[-gap:]  # gap ... B-1
            M2 = masks[:gap]  # 0 ... B-1-gap
        flow = flow.to("cuda")

        # print("M2.shape:",M2.shape)
        M1, M2 = M1[ok].to("cuda"), M2[ok].to("cuda") #输出的MASK
        # print("M2.shape",M2.shape)
        F12, O12 = F12[ok].to("cuda"), O12[ok].to("cuda") #输入的光流图和遮挡图
    ...
    def forward_old(self, batch_in, batch_out, split=False):
        """
        We consider point A in frame 1. Flow takes A to B in frame 2: FLOW_12(A) -> B.
        The mask value of point A in 1 should be the same as the mask value of point B in 2,
        unless point A is occluded.
