...
def compute_video_mean_std(video_img_dir):
    ...
    for img_name in os.listdir(video_img_dir):
        ...
        # 强制转为灰度图，并转换为 numpy 数组，归一化到 [0,1]
    ...
    std = np.sqrt(max(variance, 1e-10))  # 防止负值
...
def train0_global(
    ...
    # 2. 定义图像transform的基本结构（如果使用全局统计量）
    # 注意：target_transform 在所有数据集上一致
    target_transform = transforms.Compose([
        transforms.ToTensor(),
        lambda x: (x > 0.5).float()   # 将标签中 255 转换为 1
    ])

    # 3. 为每个视频创建数据集，可能使用各自的归一化参数
    datasets = []
