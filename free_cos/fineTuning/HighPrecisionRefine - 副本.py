...
class HighPrecisionRefine:
    ...
    def run(self):
        ...
        if self.use_otsu:
            ...
            print(f"  前向幅值阈值: {th_mag:.4f}, 误差阈值: {th_err:.4f}")
        else:
            # 固定阈值模式（可自定义）
            th_mag = 5.0
            th_err = 10.0
            ...
            high_err = err > th_err
            print(f"  固定幅值阈值: {th_mag:.4f}, 误差阈值: {th_err:.4f}")

        # 高置信度运动区域（AND）
        if self.motion_combine == 'and':
            high_motion = high_mag & high_err
            print("运动区域组合: 高幅值 AND 高误差")
        elif self.motion_combine == 'or':
            high_motion = high_mag | high_err
            print("运动区域组合: 高幅值 OR 高误差")
        else:
        ...
        print(f"修正后掩码已保存至: {self.output_path}")
...
def getPrecisionMask():
    ...
    for userId in os.listdir(datasetPath):
        ...
        if not os.path.isdir(user_img_dir) or not os.path.isdir(user_gt_dir):
            print("!!!not os.path.isdir(user_img_dir) or not os.path.isdir(user_gt_dir):",user_img_dir,user_gt_dir)
            continue
        for videoId in os.listdir(user_img_dir):
            ...
            for frameId_png in os.listdir(img_dir):
                ...
                gt_mask =   os.path.join(datasetPath, "ground_truth", videoId + "CATH", + frameId + ".png")
                ...
                refiner = HighPrecisionRefine(
                    ...
                    use_otsu=True,
                    motion_combine='and',           # 必须 'and' 以获得最高精度
                    keep_largest_only=True,          # 只保留最大连通组件
                    min_component_area=200,          # 当 keep_largest_only=False 时有效
                    morph_kernel=3
                )
                refiner.run()
...
if __name__ == "__main__":
    ...
    videoId = "CVAI-1207LAO44_CRA29"
    frameId = "00036"

    fwd_flo = "./raw_flows_gap1/" + videoId + "/" + frameId + ".flo"
    bwd_flo = "./raw_flows_gap-1/" + videoId + "/" + frameId + ".flo"
    pred_mask = "./_25-01.nir/" + videoId + "/" + frameId + ".png"
    gt_mask = "./ground_truth/" + videoId + "CATH/" + frameId + ".png"
    output_mask = "./high_precision_refine/" + videoId + "/" + frameId + ".png"
