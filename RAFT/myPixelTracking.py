import os
os.environ['KMP_DUPLICATE_LIB_OK'] = 'TRUE'  # 必须在导入问题库之前设置！
...
class Track():
    ...
    def getInv(self,masks, gap):
        masks = masks[:,[[0]]]
        batch_in = self.batch
        if gap > 0:  # 正向光流 #光流数据:整个图片空间的范围是[-1,+1],图片光流图中的数据为每个像素点的坐标位移
    ...
    def initTrack(self,imgs0): # imgs = [10, 1, 128, 128] # imgs0.shape=torch.Size([92, 3, 512, 512])
        ...
        # 针对这组图片预测出所有情况
        imgs0=imgs0[:,[0]] # [10, 1, 128, 128] <= [10, 3, 128, 128] #将三通道变为单通道
...
if __name__ == "__main__":
    ...
    fwd_dir = os.path.join(ROOT, outpath, tag + "_fwd")
    bck_dir = os.path.join(ROOT, outpath, tag + "_bck")
    mask_dir = os.path.join(ROOT, outpath, tag + "_filter")
    Track(rgb_dir, fwd_dir, bck_dir, mask_dir)
