'''
    内容：
        只对刚体使用微弱的不确定性(不确定联合方差的影响) #我猜测不确定性会增加刚体层的权重
    结果：
        指标下降的幅度似乎并不微弱
    分析：
        代码写错了
        指标下降原因分析:
            方差的中心不是1, 要更小一些
        如果方差非常小：
            则重构约束会非常大、导致其他约束的作用被削弱
    实验设备: AutoDL_P、DeNVeR.26-3_new
    Running time: 约18.5 hours
'''
config_A26_03_01P1={ # follow: config_A26_03_01J2
            "decouple":{ # 解耦
                "tag":"A26_03_01P1",
                "de-rigid":"1_sim",#去噪框架
                ...
                "UncertainLearning":{
                    ...
                    "activationFunctionRadius":0.05,#1, #只有当激活函数类型为sigmoid的时候才生效
                    "var_dias":0,#1,#默认为0
                    "weitht_all":2,#2,#默认为1
                    "weight_regular":{
                        "ra":1, #默认为1
