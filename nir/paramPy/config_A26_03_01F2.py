'''
    ...
    结果:
        与预测相反
        关闭软体运动后指标大幅下降
        软体为啥会干扰刚体：(也不确定、毕竟没有低于下限)
            似乎在使用PE编码后会增强学习能力(不确定),而且这里软体和刚体使用了相同的运动
    分析:
        结果非常奇怪  
...
config_A26_03_01F2={ #follow 25: config_A26_03_01F
            "decouple":{ # 解耦
                "tag":"A26-03-01F2",#这里测试的时候写错了，写成了"A26-03-01"
                "de-rigid":"1_sim",#去噪框架
                ...
                "configSofts":{ # 软体
                    "layer":{
                        ...
                        'hidden_features_global':0,#8*128, 
                        # 2.局部运动
                        "useLocal":False,#True,
                        'hidden_layers_local':0,#2,#2,
                        'hidden_features_local':0,#8*128,#8*128, # Mask遮挡
                        # 3.纹理
                        "dynamicTex":False, #动态纹理
            ...
            "name": "A26-03-01F2", #提高模型的拟合能力
            "precomputed": False,
            "noise_label":"A26-03-01F2.rigid",
            "input_mode": "A26-03-01F2.rigid.non1",
