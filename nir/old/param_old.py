...
# def main(): 
    ...
    configs = [#在短视频数据上的测试结果
        ...
        # --test10: 背景损失函数 MSE=>myLog 软体对应的损失函数变大了、但是软体拟合效果没有提升
        # --test11: 两个刚体层
        # 为了避免静止时的抖动、
        { #流体层进行渐进式学习
            "decouple":{ # 解耦
                "tag":"A27-09.moveRigid2",
                "de-rigid":"1_sim",#去噪框架
                # "total_steps":2000,#1000,#"epoch":1000,#2000,#2000,#6000,#4000,#2000,          
                # 只兼容了startDecouple1 #recon_all=0.00011
                "epochs":0.625,#欠拟合和过拟合都没太大差别
                "batch_size_scale":0.5,#1/8,
                "dynamicVesselMask":{#有较长的时间开销
                    # "startEpoch":1000*10,
                ...
                "configRigids":{ # 整个刚体层模块的参数
                    "layer":{
                        ...
                        "dynamicTex":False, #动态纹理
                        'hidden_layers_map':4,#1,#2,#4,#32,#4,
                        # 'hidden_features_map': 8*512,#8*512,#256,#64,#8,#2*4*512,#16*4*512,#128,#512, #128,
                        'hidden_features_map': 8*512,
                        "posEnc":False,
                        "use_featureMask":False,
                    },
                }, 
                ...
                "configSofts":{ # 软体
                    "layer":{
                        ...
                        "dynamicTex":True, #动态纹理
                        'hidden_layers_map':2, # 1, # 2, # 4, # 32, # 4,
                        'hidden_features_map': 512,#8*512, # 将隐含层特征维度变为1/8
                        "posEnc":False,
                ...
                # },
                # 1.3 流体模块
                "NUM_fluid":1,#1, # 0.00019 -> 0.00016、0.00015
                "configFluids":{ #参数数量
                    "layer":{
                        "use_residual":{
                            "R":False,
                        ...
                        "use_featureMask":True,#False,#不进行遮挡的话流体层的信息太多了
                        "fm_total_steps":800/2000, #use_featureMask=true的时候启用
                    },        
                    #######################
                    "vesselMaskInference":True,#False,
                ...
                "lossParam_vessel":{ #只训练血管时的参数 #这里未被使用
                    "ra":"F",  #只有这里有效
                    "rm":None, #固定为None
                    "rv":None, #固定为None
                    }, 
                "lossFunType":{ #无法只拟合血管 #"MSE", "myLog", "atten_d"
                    ...
                    "rv":"myLog",#"MSE", #更模糊一些
                    "rv_eps":0.5,#0.1,#该参数的效果还没有被测试 #训练不足
                    "vesselMask_eps":1,#0.1,#0.25, #用于对流体输出的遮挡计算
                }, 
                "maskPath_pathIn":None,#"A20-10-best1.rigid.non1", # 当"rm"==None的时候,没有用处 #是否使用预先计算好的MASK
                "useMask":True, #只有lossType==1的时候才有效
                ########################
        ...
        #         "de-soft":None,
        #     },
        #     "name": "A27-09.Fluid2", #提高模型的拟合能力
        #     "precomputed": False,
        #     "noise_label":"A27-09.Fluid2.rigid",
        #     "input_mode": "A27-09.Fluid2.rigid.non1",
        #     "norm_method": norm_calculator.calculate_mean_variance,
        #     "binarize": True,
