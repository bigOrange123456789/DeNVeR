...
class ModelVessel(nn.Module):
    def __init__(self, path,maskPath="./nir/data/mask/filter"):
        super().__init__()
        v = VideoFitting(path,maskPath=maskPath)
        videoloader = DataLoader(v, batch_size=1, pin_memory=True, num_workers=0)
        model_input, ground_truth, maskBG = next(iter(videoloader))
        model_input, ground_truth, maskBG = model_input[0].cuda(), ground_truth[0].cuda(), maskBG[0].cuda()
        ground_truth = ground_truth[:, 0:1]  # 将RGB图像转换为灰度图

        self.v=v
        self.model_input=model_input
        self.ground_truth = ground_truth
        self.maskVessel = 1-maskBG
    ...
    def loss(self, xyt, step, start, end):
        ...
        if self.v.useMask:
            loss_recon = loss_recon*self.maskVessel[start:end]
            loss_recon = loss_recon.sum()/(self.maskVessel[start:end].sum()+1e-8)
        else:
