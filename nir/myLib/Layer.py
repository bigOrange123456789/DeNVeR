...
class Layer2(nn.Module): #用于表示软体层和流体层、能够实现PE和纹理featureMask
    def _getFeatureMask(self, k, tag):#tag=motion运动 或 texture
        dim = self.config["hidden_features_map"] if tag=="texture" else max(self.config["hidden_features_global"],self.config["hidden_features_local"])
        # print("k, tag, dim:",k, tag, dim)
        ...
        if self.getFeatureMask["type"]=="sigmoid":
            return torch.clamp(vec, 0, 1)
        return  torch.sigmoid( (2*vec-1)**3 )#输入sigmoid所以要将区间范围由[0,1]变为[-1,1]
    
    def getFeatureLen(self): #用于计算损失函数
        # self.config["hidden_features_global"],self.config["hidden_features_local"],self.config["hidden_features_map"]
        featureLen_motion = max(self.config["hidden_features_global"],self.config["hidden_features_local"]) #整体数量级取决于局部运动和整体运动跨度最高的那个
        featureLen_texture = self.config["hidden_features_map"]
        return featureLen_motion, 0.1 * featureLen_texture
    
    def kFeatureMaskUpdate(self,methodType): #基于二分查找思想的快速更新机制
        ...
        def getNewParam(now, config):
            if methodType==0: #废弃
                ...
                return config["pre"]
            elif methodType==1: #目前使用
                now = now.get()
                pre = config["pre"]
                if not pre is None:
                    if now > pre:#应该变大
                        config["min"] = pre
                        if config["max"]<pre: config["max"]=(pre+1)/2
                        now = ( config["min"] + config["max"] )/2
                    elif now< pre: #应该变小
                        config["max"] = pre
                        if config["min"]>pre: config["min"]=(0+pre)/2
                        now = ( config["min"] + config["max"] )/2
                config["pre"] = now
                return now
            else:
                print("Err: ./nir/myLib/Layer.py: class Layer2(): def kFeatureMaskUpdate(): methodType")
                exit(0)
    ...
    def kFeatureMaskUpdate2Level(self,SN_J): #更新到几个level
        # SN_J #这几个数值必须从大到小排列
        def getNewParam(now, config):
                now = now.get()
                pre = config["pre"]
                if not pre is None:
                    if now > pre:#应该变大
                        # now = min(x for x in SN_J if x >= now)
                        greater = [x for x in SN_J if x >= now]
                        # print("greater:",greater)
                        if len(greater)>0:# 如果有大于 t 的元素
                            now = min(greater)
                        else:
                            now = max(SN_J)
                    elif now< pre: #应该变小
                        # now = max(x for x in SN_J if x <= now)
                        greater = [x for x in SN_J if x <= now]
                        if len(greater)>0:# 如果有小于 t 的元素
                            now = max(greater)
                        else:
                            now = min(SN_J)
                config["pre"] = now
    ...
    def __init__(self, # useGlobal=True, useLocal=True,  useMatrix=True, #废弃
                ...
                use_dynamicFeatureMask=False,
                init_dynamicFeatureMask=[0,1],#None,
                getFeatureMask=None,
                config={},
                #平滑损失参数
        ...
        self.getFeatureMask=getFeatureMask
        if not config is None:
            # print(config)
        ...
        self.config = config #self.config["hidden_features_global"],self.config["hidden_features_local"],self.config["hidden_features_map"]
        ...
        if self.useGlobal:
            self.g_global = Siren(in_features=in_time_dim,
                                  ...
                                  hidden_features=hidden_features_global,#16, 
                                  hidden_layers=hidden_layers_global,#2,
                                  use_residual=config["use_residual"]["R"],#["use_residual"]["G"],
                                  outermost_linear=True)
            self.parameters.append(self.g_global.parameters())
            self.g_global.cuda()
        #################################### 三、局部运动代码 ####################################
        ...
        if use_dynamicFeatureMask:
            ...
            self.kFeatureMask_motion = LearnableVariable(
                init_dynamicFeatureMask[0]#0 if init_dynamicFeatureMask is None else init_dynamicFeatureMask
                ) #nn.Parameter(torch.tensor(1, dtype=torch.float32).cuda())
            # print("init_dynamicFeatureMask",init_dynamicFeatureMask)
            # print("self.kFeatureMask_motion",self.kFeatureMask_motion)
            self.parameters.append(self.kFeatureMask_motion.parameters())
            self.kFeatureMask_texture = LearnableVariable(
                init_dynamicFeatureMask[1]#1
                ) #nn.Parameter(torch.tensor(1, dtype=torch.float32).cuda())
            self.parameters.append(self.kFeatureMask_texture.parameters())
    ...
    def _getSmoothLoss(self,xyt, h_global):
        ...
        if self.useSmooth:#>0
                ...
                if self.useSmooth==1: #运动速度趋于0 #失败:无法迫使刚体正确运动
                    loss_smooth = jacobian(h_global, t).abs().mean()
                ...
                elif self.useSmooth==7:
                    dt = interval0*0.001
                    y0 = h_global
                    y1 = self.g_global(t+dt)#[:,:2] #后面
                    speed = (y1-y0)/dt
                    loss_smooth = speed.abs().mean() 
                elif self.useSmooth==8:
                    dt = interval0
                    y0 = self.g_global(t-dt)
                    y1 = h_global
                    y2 = self.g_global(t+dt)
                    dy = 2*y1-y0-y2 #(y1-y0)-(y2-y1)
                    loss_smooth = (dy**2).mean() *(10**6)
                    # print("y",y0.mean(),y1.mean())
    ...
    def forward(self,xyt,current_step):  #这部分是整个程序的核心       
        ...
        # 0.特征遮挡向量 
        '''
            在旧版中: 动态作用于运动, 渐进式只作用于纹理
        '''
        if self.use_dynamicFeatureMask: #自适应
            featureMask_motion=self._getFeatureMask(#动态参数
                self.kFeatureMask_motion(),#.detach().clone() #这里必须进行梯度回传, 因此不能进行detach
                "motion", 
            ) 
            featureMask_texture=self._getFeatureMask(#动态参数
                self.kFeatureMask_texture(),#.detach().clone() #这里必须进行梯度回传, 因此不能进行detach
                "texture", 
            ) 
        elif self.use_featureMask: #渐进式
            if current_step is None: # 推理的时候这个似乎是None
                k=1
            else:
                k=current_step/self.config["fm_total_steps"]
            featureMask_texture = self._getFeatureMask(k,"texture")
            featureMask_motion = None
        else: 
            featureMask_motion = None
            featureMask_texture = None
        
        # 1.整体运动 
        if self.useGlobal:
            c =self.g_global(pet(xyt[:, [-1]]), featureMask=featureMask_motion)
            tx = c[:, 0]  # X轴位移
            ty = c[:, 1]  # Y轴位移
            if False:
                t = xyt[:, -1]
                # print(t.shape,tx.shape)
                # exit(0)
                tx[t<-0.5]=0
                ty[t<-0.5]=0
            u = xyt[:, 0]
        ...
        if self.useLocal:
            # 2.2 计算局部位移
            h_local = self.g_local(pe(xyt), featureMask=featureMask_motion)
            # 2.3 限制形变程度
            if self.useDeformation:
                h_local=2*torch.sigmoid(h_local)-1
                h_local=h_local*self.deformationSize
        ...
        out=self.f_2D(
            combined_in,
            featureMask=featureMask_texture
        )
        # print("out",out.shape,type(out))
        color = torch.sigmoid(out[:,[0]])
        if out.shape[1]==2:#使用不确定学习，第一个维度表示颜色均值、第二个维度表示颜色标准差
            var0 = out[:,[-1]]
            if self.UncertainLearning["activationFunction"]=="softplus":
                var = nn.functional.softplus(var0) + 1e-10 #该模型下方差的最小值是1e-10
            elif self.UncertainLearning["activationFunction"]=="square":
                var = var0**2
            elif self.UncertainLearning["activationFunction"]=="sigmoid":
                var = self.UncertainLearning["activationFunctionRadius"] * ( torch.sigmoid(var0) + 0.5 ) # 值域: [0,1]=>[0.5,1.5]
            elif self.UncertainLearning["activationFunction"]=="sigmoid2":
                var = self.UncertainLearning["activationFunctionRadius"] * ( torch.sigmoid(var0) - 0.5 ) + 1 # 值域: [0,1]=>[1-x/2,1+x/2]
        ...
        # print("out",out.shape,type(out),"color",color.shape) #out torch.Size([78643, 2]) <class 'torch.Tensor'> #out torch.Size([78643, 1]) <class 'torch.Tensor'> color torch.Size([78643, 1])
        # print("std",std.max(),std.min(),type(std),std.shape)
    ...
    def forward_old(self,xyt,current_step):  #这部分是整个程序的核心       
        def pep(xy_0):
            return self.pos_encoder(xy_0, current_step) if self.use_PE else xy_0
        def pet(t0):
            return self.time_encoder(t0, current_step) if self.use_PE else t0
