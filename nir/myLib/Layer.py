...
class Layer2(nn.Module): #用于表示软体层和流体层、能够实现PE和纹理featureMask
    def _getFeatureMask(self, k, tag):#tag=motion运动 或 texture
        dim = self.config["hidden_features_map"] if tag=="texture" else self.config["hidden_features_local"]
    ...
    def getFeatureLen(self): # self.config["hidden_features_global"],self.config["hidden_features_local"],self.config["hidden_features_map"]
        ...
        return featureLen_motion, featureLen_texture
