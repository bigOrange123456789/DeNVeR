...
#     """生成彩色图片"""
    ...
    def generate_comparison_matrix_figure(self, configs, save_path="", block_cath=False, threshold=0.85):
        """
        生成N*N比较矩阵图，每个小图显示彩色MASK图
        ...
        all_config_predictions = self._collect_all_config_predictions(
            configs, threshold, block_cath
        )
        
        # 步骤2: 生成比较矩阵图
        print("步骤2: 生成比较矩阵图...")
        matrix_img_path = f"{save_path}comparison_matrix_color{mode_suffix}_{timestamp}.jpg"
        self._create_color_comparison_matrix_plot(all_config_predictions, configs, matrix_img_path)
        
        print("\n" + "=" * 80)
        print("比较矩阵图生成完成!")
        print(f"✓ 彩色比较矩阵图已保存: {matrix_img_path}")
        
        return matrix_img_path
    
    def _collect_all_config_predictions(self, configs, threshold=0.85, block_cath=False):
        """
        收集所有配置的预测结果
    ...
    def _process_single_image_predictions(self, patient_id, video_id, frame_id, configs, 
        ...
        for config in configs:
            ...
            # 获取预测结果
            pred = self._get_prediction(config, model, img_norm, threshold, patient_id, video_id, frame_id)
...
def main():
    ...
    # 生成彩色比较矩阵图
    comparator.generate_comparison_matrix_figure(
        configs, config.root_path + "/", block_cath, threshold
    )
    
    # # 执行多配置比较
    # if len(configs) > 1:
    #     comparator.analyze_multiple_configs(
    #         configs, 1, threshold, config.root_path + "/", block_cath
    #     )
    
    # # 执行单对比较
    # if len(configs) >= 2:
    #     comparator.analyze_pair(configs[0], configs[1], topK, threshold, config.root_path + "/", block_cath)
